...
void d2oSetup(HWND tWindow) 
{
	...
	wc.lpszClassName = "Menm12";
	RegisterClass(&wc);
	overlayWindow = CreateWindowEx(NULL,
		wc.lpszClassName, "BrubMenm", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
		NULL, NULL, wc.hInstance, NULL);

	// part of removing top-most
	SetWindowLong(overlayWindow, GWL_EXSTYLE, WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW);
	gHwnd = FindWindowA(NULL, "EscapeFromTarkov");
...
void mainLoop()
{
	...
	if (message.message != WM_QUIT)
	{
		...
		// removing TOP-MOST flag
		HWND hwnd_active = GetForegroundWindow();
		if (hwnd_active == gHwnd)
		{
